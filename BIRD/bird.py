...
_metrics2 = collections.defaultdict(tf.metrics.Mean)
...
def define_config():
    ...
    config.expl_min = 0.0
    config.ent_alpha = 1e-8
    config.reweight_clip = 1.0
    config.buffersize = 100

    return config


class BIRD(tools.Module):
    ...
    def _scalar_summaries(
        ...
        self._metrics['entropy_loss'].update_state(entropy)
        ent = self._actor(feat).entropy()
        self._metrics['action_ent'].update_state(ent)
        self._metrics['ent_max'].update_state(tf.reduce_max(ent))
        self._metrics['ent_min'].update_state(tf.reduce_min(ent))
    ...
    def _image_summaries_eager(self, data, embed, image_pred):
        truth = data['image'][:6] + 0.5
        recon = image_pred.mode()[:6]
        init, _ = self._dynamics.observe(embed[:6, :5], data['action'][:6, :5])
        init = {k: v[:, -1] for k, v in init.items()}
...
def load_dataset(directory, config):
    ...
    generator = lambda: tools.load_episodes(
        ...
        config.dataset_balance,buffersize=config.buffersize)
...
def main(config):
    ...
    # print(device)
    tf.config.experimental.set_visible_devices(devices=device[config.device], device_type='GPU')
    ...
    agent = BIRD(config, datadir, actspace, writer)
