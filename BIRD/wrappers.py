...
class Atari:
    ...
    def __init__(
        ...
        with self.LOCK:
            ...
            # self._env = gym.make('PongNoFrameskip-v0')
...
class Collect:
    ...
    def __init__(self, env, callbacks=None, precision=32, nec=False, config=None):
        self._env = env
    ...
    def step(self, action, agent_state=None, action_log_prob=None):
        ...
        if self._nec:
            if agent_state:
                latent = agent_state
                transition.update({'mean': latent['mean'][0].numpy()})
                transition.update({'std': latent['std'][0].numpy()})
                transition.update({'stoch': latent['stoch'][0].numpy()})
                transition.update({'deter': latent['deter'][0].numpy()})
                transition.update({'prev_action': action})


            else:
                transition.update({'mean': np.zeros([self._config.stoch_size])})
                transition.update({'std': np.zeros([self._config.stoch_size])})
                transition.update({'stoch': np.zeros([self._config.stoch_size])})
                transition.update({'deter': np.zeros([self._config.deter_size])})
                transition.update({'prev_action': np.zeros(self._env.action_space.shape)})

            if action_log_prob:
                transition.update({'action_log_prob': np.array([action_log_prob.numpy()])})
            else:
                transition.update({'action_log_prob': np.array(
                    [-0.69])})
...
class OneHotAction:
    ...
    def step(self, action):
        ...
        # ? question, why do we need this all_close afterall. will do experiment. 
...
class RewardObs:
    ...
    def step(self, action, agent_state=None, action_log_prob=None):
        obs, reward, done, info = self._env.step(action, agent_state, action_log_prob)
        obs['reward'] = reward
        return obs, reward, done, info
...
class Async:
    ...
    def step(self, action, blocking=True, agent_state=None, action_log_prob=None):
        return self.call('step', action, blocking=blocking, agent_state=agent_state, action_log_prob=action_log_prob)
