...
class WorldModel(common.Module):
  ...
  def imagine(self, policy, start, horizon):
    ...
    # !
    start['prob'] = tf.zeros([start['logit'].shape[0]], tf.float32)
    ...
    # !
    # ? here I'm taking the first 14 elements, but need to figure out the exact size
    # prob = succs.pop('prob')
    # prob = prob[:-1]
    # start.pop('prob')
    ...
    # print(action.shape)
    # print(start)
    # prob = common.static_scan(self.rssm.cal_prob, action, start)
    # print(prob)
    # print(prob.shape)
    # exit()
    ...
    return feats, states, actions, discount, succs['prob']
    # return feats, states, actions, discount, 1
...
class ActorCritic(common.Module):
  ...
  def train(self, world_model, start, reward_fn):
    ...
    with tf.GradientTape() as actor_tape:
      ...
      feat, state, action, disc, prob_traj = world_model.imagine(self.actor, start, hor)

      # prob = world_model.rssm.get_dist(state)
      # print(prob)
      # print(prob.shape)
      # exit()
      ...
      # ? clipping threshold needs some experiments later
      prob_traj = tf.clip_by_value(prob_traj, 0, 2)


      reward = reward_fn(feat, state, action)
      target, weight, mets1 = self.target(feat, action, reward, disc)
      actor_loss, mets2 = self.actor_loss(feat, action, target, weight, alset_re, prob_traj)
  ...
  def actor_loss(self, feat, action, target, weight, alset_re, prob_traj):
    metrics = {}
    ...
    # !
    alset_re.append(-tf.reduce_mean(tf.stop_gradient(prob_traj)* weight[:-1] * objective))
    actor_loss = tf.reduce_mean(tf.stack(alset_re))
