...
class RSSM(common.Module):
  ...
  def imagine(self, action, state=None):
    ...
    action = swap(action)
    # !
    tmp_state = {k: v for k, v in state.items()}
    tmp_state['prob'] = tf.zeros([state['logit'].shape[0]], tf.float32)

    prior = common.static_scan(self.img_step, action, tmp_state)
  ...
  def get_feat(self, state):
    ...
    if self._discrete:
      ...
      # print('stoch shape is ', stoch.shape)
      # print('other shape are', self._stoch, self._discrete, shape)
      # print(shape, stoch.shape)
  ...
  def img_step(self, prev_state, prev_action, sample=True):
    ...
    stoch = dist.sample() if sample else dist.mode()
    # !
    prob = dist.log_prob(stoch)
    prior = {'stoch': stoch, 'deter': deter, 'prob': prob, **stats}
...
# ! new function for just cal prob for stoch
  @tf.function
  def cal_prob(self, prev_state, prev_action, sample=True):
    prev_stoch = self._cast(prev_state['stoch'])
    prev_action = self._cast(prev_action)
    if self._discrete:
      shape = prev_stoch.shape[:-2] + [self._stoch * self._discrete]
